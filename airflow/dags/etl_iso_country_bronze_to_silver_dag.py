...
def etl_iso_country_bronze_to_silver_dag():
    ...
    def transform_load_iso_country_bronze_to_silver():
        ...
        try:
            ...
            # Rename columns to match the Silver zone schema
            transformed_df = df\
            ...
            # Select only the necessary columns for the Silver zone
            ...
            load_to_s3_lakehouse(transformed_df, catalog_name, loaded_schema_name, loaded_table_name, warehouse_iceberg_directory)
    ...
    def load_to_s3_dim_iso_country_profiles():
        try:
            # Create Spark session
            spark = create_spark_session(app_name="Load_Dim_ISO_Country_Profiles")
